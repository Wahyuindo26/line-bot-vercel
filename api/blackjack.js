...
const admins = {
  pavinendra: 'Ud5a0a393d90b60e1ed170f31422ff11d'
};
...
const playerStatus = {};
const gameHistory = [];
const userActivity = {}; // 🕵️ Track user last seen/message
...
function buatFlexHasil(p1, p2, nama1, nama2) {
  ...
  if (isBust1 && !isBust2) {
    ...
    if (total1 > total2) pemenang = `${nama1} menang 🏆`;
    else if (total2 > total1) pemenang = `${nama2} menang 🏆`;
  }
...
async function handleEvent(event) {
  try {
    ...
    // 🕵️ Track aktivitas user
    const now = Date.now();
    userActivity[userId] = userActivity[userId] || {};
    userActivity[userId].lastSeen = now;
    userActivity[userId].lastMessage = now; // Karena dia mengirim pesan saat ini
    ...
    if (msg === '/hit') {
      ...
      if (total > 21) {
        ...
        await Promise.all([
          client.pushMessage(p1, { type: 'text', text: '🎮 Ronde telah selesai. /gabung untuk main lagi.' }),
          client.pushMessage(p2, { type: 'text', text: '🎮 Ronde telah selesai. /gabung untuk main lagi.' })
        ]);
...
async function mainkanGiliranBot(groupId) {
  ...
  if (total < 17) {
    ...
    if (newTotal > 21) playerStatus[botLawanId] = 'bust';
  } else {
